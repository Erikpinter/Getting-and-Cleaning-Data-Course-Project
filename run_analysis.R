...
features <- read.table("./UCI HAR Dataset/features.txt", sep = " ", col.names = c("num","name"), stringsAsFactors = FALSE, strip.white = TRUE)
features$name <-gsub("\\()", "", features$name)     # delete brackets "()" from names  
features$name <-gsub("-", "", features$name)        # delete dashes "-" from names 
features$name <- make.names(features$name)          # create names

# filter only the mean and standard deviation variable names from the features --> will be used to filter data accordingly
